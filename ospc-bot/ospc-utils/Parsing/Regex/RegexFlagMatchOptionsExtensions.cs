namespace OSPC.Utils.Parsing.RegularExpressions
{
	...
	{
		private static RegexFlagMatchOptions[] appendableFlags = [
		...
		{
			if (options == RegexFlagMatchOptions.NoInput) return string.Empty;
			...
			{
				if (options.HasFlag(flag)) regex += flag.GetRegexForFlag();
			}
	
			if (options.HasFlag(RegexFlagMatchOptions.Multi)) return $@"(?<{name}>([{regex}]+)|(?<{name}>""[\s{regex}]+""))";
			else return $"(?<{name}>[{regex}]+)";
		}
