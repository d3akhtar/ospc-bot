...
namespace OSPC.Utils.Parsing.RegularExpressions
{
	public class RegexEvaluator
	{
		...
		{		
			...
			private Result<NumberComparisonValue> GetNumberComparisonValueFromMatchCollection(MatchCollection matches)
			{
				switch (matches.Count) {
					case 1: return new NumberComparisonValue(Name, matches[0].Groups[Name + "_comparison"].Value, matches[0].Groups[Name + "_value"].Value);
					case 2: {
		...
		private Dictionary<string, RegexGroup> _nameToRegexGroupMap = new();
	
		private Dictionary<RegexGroupType, List<RegexGroup>> _allRegexGroupsForGroupType = new() {
			{ RegexGroupType.Group, new() },
			{ RegexGroupType.Flag, new() },
			{ RegexGroupType.Comparison, new() }
		};
	
		private List<IRegexGroupLimitation> _limitations = new();
		private HashSet<string> _groupNames = new();
		private HashSet<string> _numberComparisonGroups = new();
		...
		{
			if (matchOptions == RegexFlagMatchOptions.NoInput) throw new ArgumentException($"Invalid match option for group");
			if (_groupNames.Contains(name)) throw new ArgumentException($"Group \'{name}\' has already been added");
		...
		{
			if (string.IsNullOrEmpty(limitation.GroupName)) limitation.GroupName = _lastGroupAdded;
		
			string name = limitation.GroupName;
			if (!_groupNames.Contains(name)) throw new ArgumentException($"Group name: {name} wasn't ever added");
		...
		{		
			...
			foreach (var limitation in _limitations.Where(l => l.NeedsOtherGroups)) {
				if (!limitation.Passes(successfulGroups)) return MatchResult.Error(limitation.ErrorMessage());
			}
		...
		private List<Group> GetSucceedingNamedGroups(MatchCollection matches)
		{
			...
			foreach (Match match in matches) {
				foreach (var name in _groupNames) {
					if (match.Groups[name].Success && !name.Any(char.IsDigit)) successfulGroups.Add(match.Groups[name]);
				}
			}
		...
		private string GetAliasGroupRegexString(string[] aliases)
		{
			...
			if (aliases.Length == 1) flags = aliases[0];
			else {
				flags += "(";
				for (int i = 0; i < aliases.Length; i++){
					flags += aliases[i];
					if (i != aliases.Length - 1) flags += '|';
				}
				flags += ")";
			}
