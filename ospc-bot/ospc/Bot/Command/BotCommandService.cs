    ...
    public class BotCommandService : IBotCommandService
    {
        ...
        public async Task<CommandResult> GetPlaycount(ChannelOsuContext ctx, string username, string beatmapLink)
        {
            ...
            if (match.Success)
                ...
                return CommandResult.Error("Invalid beatmap link format");
        ...
        public async Task<CommandResult> GetPlaycount(ChannelOsuContext ctx, string username, int beatmapId)
        {
            ...
            if (!bpcResult.Successful)
                return CommandResult.Error(bpcResult.Error!);
            else
            {
        ...
        public async Task<SearchResult> Search(ChannelOsuContext ctx, SearchParams searchParams)
        {
            ...
            if (!userResult.Successful)
                return SearchResult.Error(userResult.Error!);
            ...
            if (!filteredMostPlayedResult.Successful)
                return SearchResult.Error(filteredMostPlayedResult.Error!);
            ...
            if (!resCountResult.Successful)
                return SearchResult.Error(resCountResult.Error!);
