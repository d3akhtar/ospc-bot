...
namespace OSPC.Tests.Attributes
{
    public class JsonFileDataAttribute : DataAttribute
    {
        ...
        private readonly string _projectRootPath = Directory.GetParent(Directory.GetCurrentDirectory())!.Parent!.Parent!.FullName;
        ...
        public override IEnumerable<object[]> GetData(MethodInfo testMethod)
        {
            if (_propertyName is null)
                _propertyName = testMethod.Name;

            if (testMethod is null)
                throw new ArgumentException(nameof(testMethod));
            ...
            if (contents.Length == 0)
                throw new ArgumentException($"Empty contents: {path}");
            ...
            foreach (JArray token in jsonArray)
            {
                ...
                for (int i = 0; i < token.Count; i++)
                    args[i] = token[i].ToObject(paramInfo[i].ParameterType, _serializer)!;
                yield return args;
            }
        }
